...
function removeMember(member: Member) {
  if (confirm(`Retirer ${member.email} du club ? Son accès sera supprimé.`)) {
    router.delete(`/membres/${member.id}`, { preserveScroll: true })
  }
}

function revokeInvitation(invitation: PendingInvitation) {
  if (confirm(`Révoquer l'invitation de ${invitation.email} ?`)) {
...
function roleLabel(role: UserRole) {
  return role === 'owner' ? 'Responsable' : 'Organisateur'
}
...
<template>
  ...
  <AdminLayout>
    <div class="mb-6">
      <h1 class="text-2xl font-bold tracking-tight text-sand-12">Membres du club</h1>
      <p class="mt-1 text-sm text-sand-11">
        Invitez des organisateurs et gérez leurs accès. Seul le responsable gère les membres.
      </p>
    </div>

    <!-- Inviter un organisateur -->
    <section class="mb-8 rounded-2xl border border-sand-6 bg-white p-5">
      <h2 class="mb-3 text-lg font-semibold text-sand-12">Inviter un organisateur</h2>
      <form class="flex flex-col gap-3 sm:flex-row sm:items-start" @submit.prevent="invite">
        <div class="flex-1">
          <label for="invite-email" class="sr-only">Adresse e-mail</label>
          <input
            id="invite-email"
        ...
        <div>
          <label for="invite-role" class="sr-only">Rôle</label>
          <select
            id="invite-role"
    ...
    <section v-if="invitations.length" class="mb-8">
      <h2 class="mb-3 text-lg font-semibold text-sand-12">Invitations en attente</h2>
      <ul class="space-y-3">
        <li
          ...
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="min-w-0">
              ...
              <p class="text-sm text-sand-10">
                {{ roleLabel(inv.role) }} · expire le {{ formatDate(inv.expiresAt) }}
              </p>
            </div>
            <button
              type="button"
    ...
    <!-- Membres -->
    <section>
      <h2 class="mb-3 text-lg font-semibold text-sand-12">Membres ({{ members.length }})</h2>
        ...
        >
          <div class="min-w-0">
            <p class="truncate font-medium text-sand-12">
              ...
              <span v-if="member.isSelf" class="text-xs font-normal text-sand-9">(vous)</span>
            </p>
            <p class="truncate text-sm text-sand-10">{{ member.email }}</p>
          </div>
          <div class="flex items-center gap-3">
            <label :for="`role-${member.id}`" class="sr-only">Rôle de {{ member.email }}</label>
            <select
              :id="`role-${member.id}`"
              :value="member.role"
