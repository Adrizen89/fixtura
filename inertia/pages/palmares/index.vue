...
const FORMAT_KEYS: Partial<Record<TournamentFormat, string>> = {
...
function formatLabel(format: TournamentFormat): string {
  ...
  return key ? t(`palmaresAdmin.format.${key}`) : format
}
...
<template>
  <Head :title="t('nav.palmares')" />

  <AdminLayout>
    <div class="mb-6">
      <h1 class="text-2xl font-bold tracking-tight text-sand-12">{{ t('nav.palmares') }}</h1>
      <p class="mt-1 text-sm text-sand-11">
        {{ t('palmaresAdmin.subtitle') }}
      </p>
    </div>
    ...
    <template v-else>
        ...
        <div class="overflow-x-auto rounded-2xl border border-sand-6 bg-surface">
          <table class="w-full border-collapse text-sm">
            <caption class="sr-only">
              {{
                t('palmaresAdmin.teamRecordsCaption')
              }}
            </caption>
            <thead>
              <tr
              ...
              >
                ...
                <th scope="col" class="py-3 pr-3 font-semibold">
                  {{ t('palmaresAdmin.colTeam') }}
                </th>
                <th
                  scope="col"
                  class="py-3 px-2 text-center font-semibold"
                  :title="t('palmaresAdmin.colAppearancesTitle')"
                >
                  {{ t('palmaresAdmin.colAppearancesShort')
                  }}<span class="sr-only">{{ t('palmaresAdmin.colAppearancesSr') }}</span>
                </th>
                <th scope="col" class="py-3 px-2 text-center font-semibold">
                  {{ t('palmaresAdmin.colFinals') }}
                </th>
                <th scope="col" class="py-3 px-2 text-center font-semibold">
                  {{ t('palmaresAdmin.colTitles') }}
                </th>
                <th scope="col" class="py-3 pl-2 pr-4 text-center font-semibold">
                  {{ t('palmaresAdmin.colWinRate') }}
                </th>
              </tr>
            </thead>
            <tbody>
      ...
      <section>
        <h2 class="mb-4 text-lg font-semibold text-sand-12">
          {{ t('palmaresAdmin.editionsHeading') }}
        </h2>
        <ul class="space-y-3">
          <li
            v-for="e in editions"
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div class="min-w-0">
                ...
                <p class="mt-0.5 text-xs text-sand-9">
                  {{ formatDate(e.eventDate) }} · {{ formatLabel(e.format) }}
                </p>
              </div>
              <div class="flex flex-col gap-1 text-sm sm:items-end">
                ...
                <span v-else class="text-sand-9">{{ t('palmaresAdmin.winnerUnknown') }}</span>
                <span v-if="e.finalist" class="text-xs text-sand-11">
                  {{ t('palmaresAdmin.finalistLabel', { name: e.finalist.teamName }) }}
                </span>
              </div>
            </div>
          </li>
